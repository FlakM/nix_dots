...
3. SABnzbd's completed downloads are available to BookOrbit at `/var/lib/sabnzbd/Downloads/complete` (read-only). Map that path to itself in the SABnzbd client; Deluge's `/var/media/...` paths also map to themselves. Test the connections and path mappings before enabling requests.
