{ pkgs, ... }: {

  users.users.backup = {
    isNormalUser = true;
    home = "/home/backup";
    description = "Backup User";
  };

  # one time setup of zfs permissions
  # sudo zfs allow backup compression,mountpoint,create,mount,receive tank/backup
  systemd.services.zfs-allow-tank-backup = {
    description = "Ensure ZFS delegated permissions for backup user";
    wantedBy = [ "multi-user.target" ];
    serviceConfig = {
      Type = "oneshot";
      RemainAfterExit = true;
    };
    script = ''
      ${pkgs.zfs}/bin/zfs allow backup compression,mountpoint,create,mount,receive,destroy,rollback,snapshot,hold,release,send tank/backup
    '';
  };

  users.users.backup.openssh.authorizedKeys.keys = [ "ssh-rsa  root@amd-pc" ];

  environment.systemPackages = with pkgs; [
    lzop
    mbuffer
  ];

}
