...

{
  ...
  systemd.services.ollama-pull-qwen = {
    ...
    serviceConfig = {
      ...
      ExecStart = pkgs.writeShellScript "ollama-pull-ocr-models" ''
        for model in glm-ocr qwen2.5vl:7b; do
          if ${pkgs-unstable.ollama}/bin/ollama show "$model" >/dev/null 2>&1; then
            continue
          fi
          until ${pkgs-unstable.ollama}/bin/ollama pull "$model"; do
  ...
  systemd.timers.ollama-pull-qwen = {
    description = "Pull OCR models after boot";
    wantedBy = [ "timers.target" ];
    timerConfig = {
      OnBootSec = "30s";
      Unit = "ollama-pull-qwen.service";
    };
  };
